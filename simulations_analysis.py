...
#For CDS mutations, if selection coeff = 0, change annotation to "synonymous", else "nonsynonymous"
...
#Initialize list of models and dfes
...
output_dir = "outputs_with_selection"
os.makedirs(output_dir, exist_ok=True)

# Check if the file already exists and is non-empty
file_path = f"{output_dir}/{dfe_id}_{model_id}_chr{chrom}_freq_differences6.csv"
if os.path.isfile(file_path) and os.path.getsize(file_path) > 0:
    ...
    for site in ts.sites():
        for mutation in site.mutations:
                ...
                #Selection coeffs and mutation types are dictionaries in case of stacked mutations
                md = mutation.metadata
                slim_mutation_details = md.get("mutation_list", [])
                mutation_types = []
                selection_coeffs = []
                for detail in slim_mutation_details:
                    mutation_types.append(detail['mutation_type'])
                    selection_coeffs.append(detail['selection_coeff'])
                                          
                #Store just the position and freq difference, as well as the mutation type and selection coefficient
                    ...
                    if (0 in (pop_a, pop_b)
                        ...
                        mutation_freqs.append({
                            ...
                            "pop_b_diff": pop_b_diff,
                             ...
                             "position": site_pos,
                            "selection_coeff": selection_coeffs,
